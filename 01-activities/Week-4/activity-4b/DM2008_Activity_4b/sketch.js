...
function mousePressed() {
  ...
  agents.push(new Agent(mouseX, mouseY, sz, speedX, speedY, shapeType));
}
...
// ============================
class Agent {
  constructor(x, y, sz, speedX, speedY, shapeType) {
    // Required properties
    this.x = x;
    this.y = y;
    this.sz = sz;
  ...
  
  {
    ...
    triangle(
      ...
      this.x - this.sz/2, this.y + this.sz/2,
      this.x + this.sz/2, this.y + this.sz/2);
  }
  }
}
...
/* ============================
   ...
   - Make one property evolve over time (size, hue, alpha, speed) done
   - Add a key or mouse interaction that changes *all* agents (loop over array)
   - Implement removal: shrink agents and splice them when too small done...?
============================= */
